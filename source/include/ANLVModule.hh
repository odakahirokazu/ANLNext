...
private:
  ANLVModule& operator=(const ANLVModule& r);
};


typedef std::vector<ANLVModule*>::iterator AMIter;
...
template<typename T>
void ANLVModule::register_parameter(T* ptr, const std::string& name)
{
...
template <typename T>
void ANLVModule::register_parameter_map(T* ptr, const std::string& name,
                                        const std::string& key_name,
                                        const std::string& key_default)
{
...
template <typename T>
void ANLVModule::add_map_value(T* ptr, const std::string& name)
{
  ModParam p(new ModuleParameter<T>(ptr, name));
...
template <typename T>
void ANLVModule::set_parameter(const std::string& name, T val) throw(ANLException)
{
...
inline
void ANLVModule::clear_array(const std::string& name) throw(ANLException)
{
  for (ModParamIter it=ModParamBegin(); it!=ModParamEnd(); ++it) {
    if (name == (*it)->name()) {
      (*it)->clear_array();
...
inline
void ANLVModule::set_vector(const std::string& name,
                            double x, double y) throw(ANLException)
{
  for (ModParamIter it=ModParamBegin(); it!=ModParamEnd(); ++it) {
    if (name == (*it)->name()) {
...
template <typename T>
void ANLVModule::set_map_value(const std::string& name, T val)
{
  try {
...
template <typename T>
inline
void ANLVModule::GetANLModuleIF(const std::string& name, T *ptr)
{
...
template <typename T>
inline
void ANLVModule::GetANLModuleIFNC(const std::string& name, T *ptr)
{
